...
- **Candidate Ad Ledger** sits above the Creative Fieldbook: a source-bound advertiser-profile snapshot (scope, reported advertiser total when directly available, Google-reported creative count, capture time, and delay caveat) beside the separate Field Station observation window. It never fabricates a campaign total by adding creative-level ranges.
